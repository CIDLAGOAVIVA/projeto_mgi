...
async def main():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        results = await crawler.arun("www.ceitec-sa.com/", config=run_config)
        ...
        for result in results:
            ...
            try:
                ...
                cur.execute(
                    "SELECT id FROM tbl_paginas_ceitec WHERE link = %s", (result.url,))
                existing_record = cur.fetchone()

                if existing_record:
                    # Update
                    cur.execute("""
                        UPDATE tbl_paginas_ceitec 
                        SET content = %s,
                            images = %s,
                            tags = %s,
                    ...
                    # Insert
                    cur.execute("""
                        INSERT INTO tbl_paginas_ceitec (content, link, images, tags)
                        VALUES (%s, %s, %s, %s)
