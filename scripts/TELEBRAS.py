...
async def main():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        results = await crawler.arun("https://www.telebras.com.br/", config=run_config)
        ...
        for result in results:
            ...
            try:
                ...
                cur.execute(
                    "SELECT id FROM tbl_paginas_telebras WHERE link = %s", (result.url,))
                existing_record = cur.fetchone()

                if existing_record:
                    # Update
                    cur.execute("""
                        UPDATE tbl_paginas_telebras 
                        SET content = %s,
                            images = %s,
                            tags = %s,
                    ...
                    # Insert
                    cur.execute("""
                        INSERT INTO tbl_paginas_telebras (content, link, images, tags)
                        VALUES (%s, %s, %s, %s)
